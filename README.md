...
`slides/` is a gitignored local artifact directory, so generated SVG/PPTX files are not
linked as tracked repo resources. The tracked protocol contracts are:

- [`docs/design/silent-auth-standard-flow.md`](docs/design/silent-auth-standard-flow.md) —
  end-to-end banking flow, Resolver→Verifier hand-off, SAS FSM, timeout table.
- [`docs/design/unified-identity-sms-security-architecture.md`](docs/design/unified-identity-sms-security-architecture.md) —
  access-tech decision, fail-closed pipeline, Strategy A/B rollout.
- [`docs/design/3gpp-spec-coverage.md`](docs/design/3gpp-spec-coverage.md) — MAP/Diameter
  operation coverage per stage.
- [`docs/design/camara-flow-diagram.svg`](docs/design/camara-flow-diagram.svg) — CAMARA
  Number Verification gateway flow.

Local pitch diagrams can be regenerated when the local `slides/` workspace is present:

```bash
python3 slides/scripts/generate_svgs.py
python3 slides/scripts/generate_svgs_v2.py
```
...
### LTE/5G — Diameter S6a + Sh (HSS)

| Message        | Role |
|----------------|------|
| ULR / ULA      | Update Location (attachment liveness) |
| NOR / NOA      | Notify |
| PUR / PUA      | Purge |
| Sh UDR / SNR   | User Data / read-only SIM-swap freshness (TS 29.328/29.329) |

## Related CAMARA / security context

- **CAMARA**: Number Verification, SIM Swap, OTP SMS, Scam Signal, KYC Match / Number Recycling.
- **CAMARA ICM/OAuth**: CIBA, 2-legged `client_credentials`, JWT bearer (`tel:` /
  `operatortoken:`), `private_key_jwt`, discovery and JWKS. Authorization code, refresh
  tokens, ID tokens and pseudonymous `sub` remain open.
...
- [`docs/design/silent-auth-standard-flow.md`](docs/design/silent-auth-standard-flow.md) — banking flow
...
- [`docs/design/3gpp-spec-coverage.md`](docs/design/3gpp-spec-coverage.md) — 3GPP spec → SAS
  stage map (TS 29.002 MAP PSI/ATI/SAI/SRI-SM; TS 29.272 S6a ULR/PUR + Sh UDR) with the
...
- [`docs/research/3gpp-ts29-272-s6a.md`](docs/research/3gpp-ts29-272-s6a.md) — Diameter
  S6a/S6d (ULR/PUR) + Sh UDR/SNR command reference for the LTE Verifier.
...
## Pitch decks (local artifacts)

`slides/` is gitignored. These decks and generator scripts live only in the local workspace
and are not tracked repo links:

- `slides/Restlink_Silent_Auth_Mix_v3.pptx` — recommended Mix v3 deck: government/bank story,
  MAP/Diameter technical detail, CAMARA API table, GSMA FASG security tables.
- `slides/Restlink_Silent_Auth_Technical_v2.pptx` — technical message-flow deck.
- `slides/Restlink_Silent_Auth_Ethiopia.pptx` — bank pitch deck.

When the local generator scripts are present:

```bash
python3 slides/scripts/generate_svgs.py      # v1 story assets
python3 slides/scripts/generate_svgs_v2.py   # v2 protocol diagrams
python3 slides/scripts/build_pptx_v3.py      # Mix v3; v2/v1 use build_pptx_v2.py / build_pptx.py
```
...
- [x] CAMARA ICM OAuth Phase 3 — CIBA client binding + `private_key_jwt`, 2-legged
      `client_credentials`, JWT bearer (`tel:` / `operatortoken:`), discovery/JWKS
- [ ] OAuth authorization code + refresh tokens + ID token + pseudonymous `sub`
