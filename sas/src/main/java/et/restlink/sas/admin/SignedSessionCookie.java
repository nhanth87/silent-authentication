...
public final class SignedSessionCookie {
    public static final String COOKIE_NAME = "sas_admin_session";
    public static final String CSRF_COOKIE_NAME = "sas_admin_csrf";
    public static final String CSRF_HEADER = "X-Sas-CSRF";
    ...
    public static String issue(String hmacSecret, String username, String role, String tenantId,
        ...
        if (hmacSecret == null || hmacSecret.isBlank()) {
            throw new IllegalStateException("sas.admin.session-hmac-secret required");
        }
    ...
    public static Optional<Claims> verify(String hmacSecret, String token) {
        ...
        if (!constantTimeEquals(expectedMac, parts[2])) return Optional.empty();
        try {
            String payload = new String(B64D.decode(parts[1]), StandardCharsets.UTF_8);
            String[] lines = payload.split("\n", -1);
            if (lines.length != 4) return Optional.empty();
    ...
    public static String csrfToken(String hmacSecret, String sessionToken) {
        if (hmacSecret == null || hmacSecret.isBlank() || sessionToken == null || sessionToken.isBlank()) {
            return "";
        }
        return B64.encodeToString(hmac(hmacSecret, "csrf." + sessionToken));
    }

    /** Double-submit: cookie and {@link #CSRF_HEADER} must both be present and equal. */
    public static boolean csrfMatches(String cookieToken, String headerToken) {
    ...
    public static Optional<String> extractCookie(String cookieHeader, String name) {
        if (cookieHeader == null || cookieHeader.isBlank() || name == null) return Optional.empty();
        String prefix = name + "=";
        for (String part : cookieHeader.split(";")) {
            String p = part.trim();
            if (p.startsWith(prefix)) return Optional.of(p.substring(prefix.length()));
        }
        return Optional.empty();
    }
