...
/**
 * Per-request correlation of S6a answers (RFC 6733 §8.1 Session-Id, §8.2
 ...
 */
final class S6aExchangeCorrelator {

    private static final class Pending {
        private final CompletableFuture<S6aAnswer> future;
        private volatile Long hopByHopId;

        private Pending(CompletableFuture<S6aAnswer> future) {
            this.future = future;
        }
    }
    ...
    CompletableFuture<S6aAnswer> register(String sessionId) {
        CompletableFuture<S6aAnswer> future = new CompletableFuture<>();
     ...
     */
    boolean complete(String sessionId, S6aAnswer answer) {
        if (sessionId == null) {
            return false;
        }
    ...
    boolean completeByHopByHop(Long hopByHopId, S6aAnswer answer) {
        if (hopByHopId == null) {
            return false;
        }
