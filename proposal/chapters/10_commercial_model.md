...
Digicom-ET proposes a **Value-Added Service (VAS) adapter** deployed in partnership with **Ethio Telecom**. The adapter exposes a CAMARA-aligned Silent Authentication Service (SAS) to **government agencies** and **commercial banks**. Revenue is generated by billing enterprise customers per successful or attempted `/verify` API call — **not** by intercepting, reselling, or replacing Ethio Telecom SMS wholesale or interconnect traffic.

This commercial structure is deliberate: it aligns incentives across regulators, the national operator, financial institutions, and Digicom-ET. Government and banks gain lower authentication friction and reduced account-takeover (ATO) exposure; Ethio Telecom retains existing SMS and data revenue streams while optionally participating in a **new** enterprise API revenue layer; Digicom-ET earns recurring platform fees for orchestration, policy, and integration.
...
### 10.2.1 What Digicom-ET is — and is not

| Aspect | Digicom-ET role | Explicitly **not** in scope |
|--------|-----------------|-----------------------------|
...
flowchart TB
    ...
    subgraph digicom [Digicom-ET VAS]
...
**Key invariant:** Digicom-ET does **not** take Ethio Telecom SMS revenue. Fallback OTP traffic is an **operator-billed** residual channel, not a Digicom margin pool.
...
| **New:** Open Gateway / API revenue share | Ethio Telecom (optional) | Additive share of Digicom enterprise `/verify` gross margin |

### 10.3.3 Optional revenue share with Ethio Telecom

Digicom-ET proposes an **optional, additive** revenue-share mechanism — not a replacement of SMS income:
...
Government contracts are expected to flow through **framework agreement** with Ethio Telecom as network anchor and Digicom-ET as VAS integrator, satisfying public-procurement visibility requirements.
...
| Call-centre IVR recovery | Out of scope Phase 1; OTP via operator SMSC | No Digicom SMS margin |

Banks continue existing **Ethio Telecom SMS contracts** for residual OTP. Digicom invoices **authentication API** separately — simplifying audit and avoiding conflict with telco A2P pricing.
...
| Silent auth unit cost (Digicom `/verify`) | $0.010 / call | **ILLUSTRATIVE** — Growth tier |
...
| Digicom platform + integration (Year 1) | $120,000 | $280,000 | $450,000 |
...
Ethio Telecom SMS revenue **declines proportionally** to successful silent-auth substitution — an expected enterprise efficiency gain, not revenue diversion to Digicom. Optional API share and increased cellular-data engagement partially offset SMS volume reduction. **Digicom does not capture the SMS delta.**
...
| **Digicom-ET** | Recurring `/verify` platform revenue | Integration + managed service margins | — |
...
|------|-------------------|
| **Master Services Agreement (MSA)** | Digicom-ET ↔ each enterprise tenant |
| **Network Attachment Agreement** | Digicom-ET ↔ Ethio Telecom (VAS hosting, Resolver access, signalling quotas) |
| **SLA** | 99.5% API availability (Pilot); 99.9% (Production); P95 latency ≤ 3s inclusive of FALLBACK signal |
| **Data processing** | MSISDN/IMSI processed in-operator; pseudonymised `reqId` in Digicom logs; no sale of subscriber data |
...
| Topic | Digicom-ET approach |
|-------|---------------------|
| **SMS bypass perception** | Public commitment: fallback OTP billed by operator; Digicom publishes traffic split quarterly |
