...
This chapter consolidates the **security, privacy, and regulatory compliance** requirements for the Digicom-ET Silent Authentication pilot in Ethiopia. It provides an implementable checklist derived from the SAS design constraints, GSMA FASG signalling rules (Chapter 8), and CAMARA Open Gateway security profile (Chapter 7). It also maps the programme at a high level to **United Nations** and **ITU** cybersecurity guidance applicable to critical digital identity infrastructure.
...
The following checklist is **mandatory** for pilot go-live. Items marked *(operator)* are Ethio Telecom responsibilities; items marked *(Digicom)* are Digicom-ET responsibilities; *(joint)* requires coordinated implementation.
...
| S-02 | PSI preferred over ATI | Use ProvideSubscriberInfo (Cat 2.1) as primary 2G/3G verifier; ATI only if operator policy permits intra-net | *(Digicom)* | FS.11 Cat 2.1 |
| S-03 | Intra-network Diameter only | S6a IDR/AIR from SAS to own HSS; no interconnect-originated subscriber queries | *(Digicom)* | FS.19 |
| S-04 | SAS inside operator trust zone | Digicom SAS hosts in operator DMZ / trusted VAS segment; no public SS7/Diameter exposure | *(joint)* | FS.31 |
...
| # | Control | Detail | Owner | Standard |
|---|---------|--------|-------|----------|
| S-09 | **mTLS bank → Digicom** | Mutual TLS on all `/verify` and CAMARA API calls; client certificate per bank/e-Gov backend | *(Digicom)* | Open Gateway profile |
| S-10 | **reqId idempotency** | Every request carries unique `reqId`; SAS deduplicates retries; one MAP/Diameter dialog per stage per `reqId` | *(Digicom)* | SAS design |
| S-11 | Anti-replay window | `ts` (timestamp) validated within configurable skew window (e.g. ±60 s) | *(Digicom)* | SAS design |
| S-12 | OAuth 2.0 / OIDC | Client credentials or JWT bearer for API authorisation layer above mTLS | *(Digicom)* | CAMARA / Open Gateway |
| S-13 | TLS 1.2+ only | No deprecated cipher suites on API endpoints | *(Digicom)* | Industry baseline |
...
|---|---------|--------|-------|
| S-15 | **Fail-closed scoring** | `assurance < threshold` → `FALLBACK`; never approve on partial evidence | *(Digicom)* |
| S-16 | CGNAT disambiguation | Require `srcIP + srcPort + ts`; reject if Resolver returns >1 MSISDN | *(Digicom)* |
| S-17 | Point-in-time binding | Resolver read keyed to request `ts`, not "latest session" | *(Digicom)* |
| S-18 | Dialog leak prevention | Every MAP dialog bounded by TC timer (2 s); timeout → `abort()`; no hung HSS queries | *(Digicom)* |
| S-19 | Total SAS budget ≤ 3 s | Resolver 300 ms; MAP/Diameter 2 s; total 3 s → `FALLBACK` | *(Digicom)* |
| S-20 | SIM-swap cooldown | Downgrade/block when `lastUpdateLocation` age < configured cooldown | *(Digicom)* |
| S-21 | Spoofed GT defence | Verifier trusts only responses from own HSS; ignore interconnect-originated answers | *(Digicom)* | FS.11 §3.3.4 |
...
| S-23 | IMSI not exposed externally | IMSI used internally by Verifier; not included in CAMARA API responses to apps | *(Digicom)* |
| S-24 | Minimal response fields | Return only `{match, assurance, reqId, fallbackRecommended}` plus optional verified MSISDN to backend | *(Digicom)* |
| S-25 | Log redaction | Application logs exclude IMSI; MSISDN hashed or tokenised in operational logs | *(Digicom)* |
...
| S-27 | Data retention limits | `reqId` audit records retained per regulatory minimum; no indefinite HSS query cache | *(Digicom)* |
...
| Data element | Mobile app | Bank backend | Digicom SAS | Ethio Telecom core |
|--------------|------------|--------------|-------------|-------------------|
...
| UN instrument / body | Relevant principle | Digicom-ET alignment |
|---------------------|-------------------|----------------------|
...
| ITU reference | Relevant principle | Digicom-ET alignment |
|---------------|-------------------|----------------------|
...
| **Telecommunications regulation** | VAS provider operates under operator agreement | Digicom as Ethio Telecom VAS partner; no independent signalling |
...
| SAS logic | Digicom engineering + operator | S-15 through S-21; FSM test suite pass |
| Privacy | Legal / DPO | S-22 through S-28; DPIA if required |
| Fallback OTP path | Ethio Telecom + Digicom | Home Routing + SG.22 policy active |
...
Digicom-ET Silent Authentication is designed for **fail-closed, operator-internal, privacy-minimising** deployment. The non-negotiable controls are: **no interconnect ATI**, **mTLS with reqId idempotency**, **MSISDN never returned to the mobile app**, and **Strategy B protection for every residual OTP**. These align with GSMA FASG signalling security, CAMARA Open Gateway API practice, and high-level UN/ITU principles for trustworthy digital identity in telecom networks. Ethiopian regulatory specifics require dedicated legal review; this chapter provides the technical and architectural foundation for that review.
