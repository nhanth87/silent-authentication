...
|------|-----------|----------------------------|
| **ATI** | AnyTimeInterrogation | SS7 MAP operation querying subscriber state and location from the HLR. **FS.11 Category 1 on interconnect** — permitted **intra-network only** in Digicom deployment. |
...
| **CAMARA** | — | Linux Foundation / GSMA telco API project. Defines Number Verification, SIM Swap, OTP SMS, and related northbound APIs. Digicom exposes CAMARA-shaped adapters over SAS. |
| **FASG** | Fraud and Security Group | GSMA working group publishing FS-series interconnect security PRDs (FS.07, FS.11, FS.19, etc.). |
| **SAS** | Silent Authentication Service | Digicom-ET component: Resolver + Verifier + Policy engine; implements `/verify` and CAMARA adapters. |
...
| **VAS** | Value-Added Service | Operator-hosted or operator-partnered service layer above core network. Digicom-ET positions as VAS adapter. |
...
|----|------|-------|-------|-------|
| OI-01 | Resolver source: PGW RADIUS accounting vs PCRF Gx/Sd vs CGNAT flow log | 1 | Ethio Telecom + Digicom | Blocks WP1.2 |
| OI-02 | jDiameter S6a client module (mirror jSS7 MAP verifier) | 2 | Digicom engineering | See Appendix B.4 |
| OI-03 | CAMARA Number Verification adapter: SAS `/verify` ↔ NV contract mapping | 1 | Digicom engineering | OpenAPI 3.1 draft in integration pack |
| OI-04 | Assurance weights + per-risk thresholds (e-Gov login vs bank transfer) | 1–2 | Digicom + tenant risk | Config-driven; no code deploy per tune |
| OI-05 | Shared identity-policy / rate-limit store (SAS + signalling FW) | 2 | Ethio Telecom + Digicom | Prevent legacy OTP abuse when silent blocked |
...
| OI-09 | NBE supervisory guidance letter for network-assisted auth | 1 | Digicom regulatory | Observer at G0 |
| OI-10 | iOS SDK entitlement / carrier privilege constraints | 3 | Digicom + Apple enterprise | May affect Wi-Fi path UX |
| OI-11 | 5G SA SMSF + SEPP path validation (FS.36) | 3 | Ethio Telecom | SAS unchanged at API layer |
| OI-12 | Disaster recovery site (secondary DC) | 2 | Digicom + operator | RPO/RTO in §11.5 WP-X.5 |
...
Digicom-ET. *Silent Authentication — Banking Flow Design*. Repository: `docs/design/silent-auth-flow.md`, 2026.

Digicom-ET. *Unified Identity & SMS Security Architecture*. Repository: `docs/design/unified-identity-sms-security-architecture.md`, 2026.

Digicom-ET. *GSMA FASG Document Index*. Repository: `docs/research/gsma-fs-index.md`, 2026.

Digicom-ET. *SMS Channel Protection Research*. Repository: `docs/research/sms-channel-protection.md`, 2026.
...
|---------|------|--------|-------------------|
| 0.1 | 2026-07-18 | Digicom-ET | Initial outline from Supermemory seed |
| 0.5 | 2026-07-19 | Digicom-ET | Commercial model + roadmap draft |
| **1.0** | **2026-07-20** | **Digicom-ET** | **Proposal release: §10–§12 for government + bank submission** |

### E.2 Approval record (draft)

| Role | Name | Signature | Date |
|------|------|-----------|------|
| Author | Digicom-ET Proposal Team | — | 2026-07-20 |
...
| Digicom-ET executive | Master | Authorisation |

### E.4 Related documents

| Doc ID | Title | Location |
|--------|-------|----------|
| DIGICOM-SA-01 | Executive summary (§1–§3) | `proposal/chapters/` |
| DIGICOM-SA-10 | Commercial model | `proposal/chapters/10_commercial_model.md` |
| DIGICOM-SA-11 | Implementation roadmap | `proposal/chapters/11_implementation_roadmap.md` |
| DIGICOM-SA-12 | Appendices (this document) | `proposal/chapters/12_appendices.md` |
| DIGICOM-SA-ROI | Illustrative ROI dataset | `proposal/assets/roi_illustrative.json` |
| DIGICOM-SA-DES | Technical design pack | `docs/design/` |
| DIGICOM-SA-DECK | Mix v3 presentation | `slides/DigicomET_Silent_Auth_Mix_v3.pptx` |
...
| SHALL / MUST | Normative requirement on Digicom design |
